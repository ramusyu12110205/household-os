...
export const billingMonth=(date,card)=>{if(!date||!card)return null;const day=Number(String(date).slice(8,10));const add=day>Number(card.close_day)?Number(card.billing_month_after_close??1):Number(card.billing_month_before_close??1);return addMonths(toMonth(date),add)+'-01'};
