...
function rulebookCacheKey(userId) { return `household-rulebook-ready:${RULEBOOK_CACHE_VERSION}:${userId}`; }
async function ensureHouseholdRulebook(userId) {
  if (rulebookReadyUserId === userId) return;
  try { if (localStorage.getItem(rulebookCacheKey(userId)) === '1') { rulebookReadyUserId = userId; return; } } catch (_) {}
  ...
  for(const [name,,linked] of HOUSEHOLD_RULEBOOK.payments.filter(x=>x[2])){if(!cardMap[linked])continue;const methodType=name==='Au Pay'||name==='楽天Pay'?'wallet':'card';const result=await supabase.from('household_payment_methods').update({method_type:methodType,linked_card_id:cardMap[linked]}).eq('user_id',userId).eq('name',name);if(result.error)throw result.error;}
  ...
  const summaries=HOUSEHOLD_RULEBOOK.summaries.map(([name,cashflow,category,subType,target])=>({user_id:userId,name,cashflow_type:cashflow,category_name:category||null,process_type:processType(cashflow,subType),target_name:target||null,archived:false}));
  ...
  for(const [name,cashflow,category,subType,target] of HOUSEHOLD_RULEBOOK.summaries){const process=processType(cashflow,subType);const patch={cashflow_type:cashflow,category_name:category||null,process_type:process,target_name:target||null};if(process==='charge'){patch.charge_source_card_id=null;if(target){const result=await supabase.from('household_accounts').select('id').eq('user_id',userId).eq('name',target).maybeSingle();if(result.error)throw result.error;patch.charge_target_account_id=result.data?.id||null;}}if(process==='card_payment'){const result=await supabase.from('household_cards').select('id').eq('user_id',userId).eq('name',target).maybeSingle();if(result.error)throw result.error;patch.withdrawal_card_id=result.data?.id||null;}const result=await supabase.from('household_summaries').update(patch).eq('user_id',userId).eq('name',name);if(result.error)throw result.error;}
  const now=new Date(),ym=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-01`;const budgets=HOUSEHOLD_RULEBOOK.budgets.map(([category,amount])=>({user_id:userId,year_month:ym,category_name:category,amount}));const {error:be}=await supabase.from('household_budgets').upsert(budgets,{onConflict:'user_id,year_month,category_name',ignoreDuplicates:true});if(be)throw be;
  rulebookReadyUserId=userId;try{localStorage.setItem(rulebookCacheKey(userId),'1')}catch(_){ }
}
