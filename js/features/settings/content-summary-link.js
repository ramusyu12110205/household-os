...
export async function enhanceContentSummaryLinks(s,refresh){
  ...
  if(!host)return;
  const {data:rules,error}=await supabase.from('household_content_rules').select('id,name,summary_ids,archived').eq('user_id',s.user.id).eq('archived',false).order('sort_order',{ascending:true}).order('name');
  if(error)return;
