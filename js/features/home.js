...
import { isExpense,isIncome,isDebtSettlement,actualAmount } from '../core/transactionRules.js';
...
export function renderHome(s){
  ...
  const repay=rows.filter(t=>isDebtSettlement(t.process_type)).reduce((a,t)=>a+actualAmount(t.amount,t.points_used),0);
  ...
  return layout('💰 家計簿OS','home',`
    <section class="card"><div class="between"><div><div class="muted small">対象年月</div><h2>${esc(m)}</h2></div><button class="primary" data-page="input">＋ 記録</button></div>
      <div class="stats"><div class="stat"><span class="stat-label">収入・借入</span><span class="stat-value">${yen(income)}</span></div><div class="stat"><span class="stat-label">実支出</span><span class="stat-value">${yen(expense)}</span></div><div class="stat"><span class="stat-label">資産移動・返済</span><span class="stat-value">${yen(repay)}</span></div></div>
