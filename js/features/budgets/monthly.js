...
export function renderMonthly(s){
  ...
  return layout('💰 家計簿OS','monthly',`<section class="card"><div class="between"><div><div class="muted small">対象年月</div><h2>${esc(m)}</h2></div><input id="month-picker" type="month" value="${esc(m)}" style="width:150px" onchange="window.__householdChangeMonth(this.value)"></div><div class="stats"><div class="stat"><span class="stat-label">収入</span><span class="stat-value">${yen(incomeTotal)}</span></div><div class="stat"><span class="stat-label">支出</span><span class="stat-value">${yen(total)}</span></div><div class="stat"><span class="stat-label">返済</span><span class="stat-value">${yen(repayTotal)}</span></div></div><div class="stat" style="margin-top:10px"><span class="stat-label">家計収支（収入−支出−返済）</span><span class="stat-value">${yen(incomeTotal-total-repayTotal)}</span></div><div class="muted small" style="margin-top:8px">振替 ${transfers.length}件・家計収支には含めません</div></section><section class="card"><h3>カテゴリ別支出</h3>${Object.entries(cats).sort((a,b)=>b[1]-a[1]).map(([n,v])=>{const b=budgets.find(x=>x.category_name===n),diff=b?Number(b.amount)-v:null;return `<div class="list-item"><div class="between"><b>${esc(n)}</b><b>${yen(v)}</b></div><div class="muted small">予算 ${b?yen(b.amount):'—'} ${diff!==null?'／ 残 '+yen(diff):''}</div></div>`}).join('')||'<p class="muted">支出なし</p>'}</section><section class="card"><h3>予算</h3><div class="between"><span>予算合計</span><b>${yen(budgetTotal)}</b></div><div class="between"><span>残額</span><b>${yen(budgetTotal-total)}</b></div></section>`);
}

window.__householdChangeMonth=async month=>{
  if(!month)return;
