...
import { renderInput } from './features/transactions/input.js';
import { renderHistory } from './features/transactions/history.js';
...
export async function refresh(){setState(await loadHousehold(state.user.id));render('home')}
export function render(page='home'){const fn=pages[page]||pages.home;$('app').innerHTML=fn(state);bind(page)}
function bind(page){document.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>render(b.dataset.page));document.querySelector('[data-action="logout"]')?.addEventListener('click',()=>supabase.auth.signOut());document.dispatchEvent(new CustomEvent('household:rendered',{detail:{page}}))}
