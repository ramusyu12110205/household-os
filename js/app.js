...
function bind(page){document.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>render(b.dataset.page));document.querySelector('[data-action="logout"]')?.addEventListener('click',()=>supabase.auth.signOut());if(page==='input')bindInput(state,()=>refresh('home'));if(page==='history')bindHistory(()=>refresh('history'));if(page==='monthly')bindMonthly(state,refresh);if(page==='settings')bindSettings(state,refresh);document.dispatchEvent(new CustomEvent('household:rendered',{detail:{page}}))}
