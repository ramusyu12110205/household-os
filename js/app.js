...
// 設定マスタの編集ボタンは、HTMLのonclick属性を解析せず、表示位置とstateを直接対応させる。
// これにより、名称に記号や引用符が含まれていても編集ボタンが壊れない。
function bindSettingsEdits(){
  const groups=[
    ['摘要',state.summaries,'editHouseholdSummary'],
    ['決済',state.payments,'editHouseholdPayment'],
    ['カード',state.cards,'editHouseholdCard'],
    ['口座',state.accounts,'editHouseholdAccount'],
    ['カテゴリ',state.categories,'editHouseholdCategory']
  ];
  const used=new Set();
  document.querySelectorAll('.list-item button').forEach(btn=>{
    ...
    const group=groups.find(g=>g[0]===label);
    if(!group)return;
    const buttons=[...document.querySelectorAll('.list-item')].filter(el=>el.querySelector('b')?.textContent?.trim()===label);
    const index=buttons.indexOf(item);
    const record=group[1]?.[index];
    const fn=window[group[2]];
    if(!record||typeof fn!=='function')return;
    btn.removeAttribute('onclick');
    btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();fn(record);});
    used.add(btn);
  });
}
